...
public class PageSpecification extends BaseSpecification<WebPage> {
  ...
  public Specification<WebPage> search(Map<String, Object> search) {
    ...
    if (search.containsKey("navigable")) {
      Boolean isNavigable = Boolean.valueOf(search.get("navigable").toString());
      specification = specification.and(
          (((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("navigable"), isNavigable))));
    }
    if (search.containsKey("showInMenu")) {
      Boolean showInMenu = Boolean.valueOf(search.get("showInMenu").toString());
      specification = specification.and(
          (((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("showInMenu"), showInMenu))));
    }
